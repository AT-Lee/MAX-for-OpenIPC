#!/bin/sh
# ============================================================================
# MAX-for-OpenIPC — one-command installer (v3.3)
#
...
#     enabled yet; if both are already on, nothing is modified or restarted.
#   * when /usr/sbin/motion.sh is missing it is created as "#!/bin/sh" plus
#     the marked max hook block; an existing motion.sh that already runs max
#     is left untouched; uninstall cuts out ONLY the marked block — the file
#     itself is never deleted; unmarked lines are not managed.
#   * the installer asks whether to enable the motion detector trigger;
#     the answer is taken from the terminal, without a terminal it defaults
#     to yes (use --motion / --no-motion for scripted installs).
...
YAML_CHANGED=0
ROLLED_BACK=0
MOTION_MODE="auto" # auto | declined | off
...
The web UI menu is patched on the current (rolling) webui only.
Missing motion.sh is created as "#!/bin/sh" plus the marked hook block;
uninstall cuts out only the marked block and never deletes the file.
...
# ----------------------------------------------------------------------------
# majestic.yaml: enable hls + motionDetect (only when they are not enabled)
# ----------------------------------------------------------------------------
yaml_enabled() { # $1 = top-level section name; returns 0 if the section exists and is enabled
        awk -v sec="^$1:" '
                $0 ~ sec { insec = 1; next }
                /^[A-Za-z][A-Za-z0-9_-]*:/ { insec = 0 }
                insec && $0 ~ /^[ \t]*enabled:[ \t]*true([ \t]|$)/ { f = 1 }
                END { exit !f }
        ' "$F_YAML"
}
...
patch_yaml() {
        ...
        h_on=0; m_on=0
        yaml_enabled hls && h_on=1
        yaml_enabled motionDetect && m_on=1
        if [ "$h_on" = "1" ] && [ "$m_on" = "1" ]; then
                skip "hls & motionDetect are already enabled in majestic.yaml — nothing to change"
                return 0
        fi
        case "$h_on$m_on" in
                01) WHAT="hls" ;;
                10) WHAT="motionDetect" ;;
                *)  WHAT="hls & motionDetect" ;;
        esac
        awk '
        ...
        YAML_CHANGED=1
        ok "enabled $WHAT in /etc/majestic.yaml (backup: majestic.yaml.bak)"
}

# ----------------------------------------------------------------------------
# /usr/sbin/motion.sh: run max on motion events (with the user's consent)
# ----------------------------------------------------------------------------
motion_answer() { # returns 0 = enable, 1 = decline
        if [ "$OPT_MOTION" = "1" ]; then
                return 0
        fi
        printf '\nEnable the motion detector trigger? A line that runs max in the\n'
        printf 'background will be added to /usr/sbin/motion.sh, so every motionDetect\n'
        printf 'event records a clip and sends it to MAX.\n'
        printf 'Enable it? [Y/n] '
        ans=""
        if ! read -r ans < /dev/tty 2>/dev/null; then
                warn "no terminal for the motion prompt — enabling it by default (use --no-motion to skip)"
                return 0
        fi
        case "$ans" in
                n*|N*) return 1 ;;
...
patch_motion() {
        ...
        if [ -f "$F_MOTION" ] && grep -q "/usr/sbin/max" "$F_MOTION"; then
                skip "motion.sh already runs max"
                return 0
        fi
        if ! motion_answer; then
                MOTION_MODE="declined"
                skip "motion trigger declined — /usr/sbin/motion.sh untouched"
                return 0
        fi
        if [ ! -s "$F_MOTION" ]; then
                {
                ...
                chmod 755 "$F_MOTION"
                ok "created /usr/sbin/motion.sh (shebang + marked max hook)"
                return 0
        fi
        [ -f "$F_MOTION.bak" ] || cp "$F_MOTION" "$F_MOTION.bak"
        awk -v b="$MARK_BEGIN" -v e="$MARK_END" -v hook="$MOTION_LINE" '
                { ln[NR] = $0 }
                $0 !~ /^[ \t]*$/ && $0 !~ /^[ \t]*#/ { last = NR }
                END {
                        ins = NR + 1
                        if (last && ln[last] ~ /^[ \t]*exit/) ins = last
                        print (ins > NR ? "APPEND" : "BEFORE-EXIT")
                        for (i = 1; i <= NR; i++) {
                                if (i == ins) { print ""; print b; print hook; print e }
                                print ln[i]
                        }
                        if (ins > NR) { print ""; print b; print hook; print e }
                }
        ' "$F_MOTION" > "$TMPD/motion.new" && [ -s "$TMPD/motion.new" ] && {
                mode=$(head -n 1 "$TMPD/motion.new")
                tail -n +2 "$TMPD/motion.new" > "$TMPD/motion.body"
                cat "$TMPD/motion.body" > "$F_MOTION"
                case "$mode" in
                        BEFORE-EXIT) ok "added the max hook to motion.sh (before the final exit; backup: motion.sh.bak)" ;;
                        *)           ok "added the max hook to the end of motion.sh (backup: motion.sh.bak)" ;;
                esac
                return 0
        }
        warn "cannot patch $F_MOTION"
}
...
patch_menu() {
        ...
        if menu_insert_after 'href="telegram.cgi"' "$ITEM" && [ -s "$TMPD/header.new" ]; then
                cat "$TMPD/header.new" > "$F_HEADER"
                ok "added MAX to the web UI menu (Services -> Notifications, after Telegram)"
                return 0
        fi
        rm -f "$TMPD/header.new"
        warn "could not find a menu anchor in header.cgi (unsupported web UI build)"
        warn "the MAX page stays reachable directly; to add a menu item manually,"
        warn "insert into /var/www/cgi-bin/p/header.cgi after the ntfy.cgi line:"
        warn "$ITEM"
...
summary() {
        ...
        case "$MOTION_MODE" in
                off)
                        ...
                        ;;
                declined)
                        printf '\n Motion mode: declined at the prompt — motion.sh untouched.\n'
                        printf '              Add /usr/sbin/max to /usr/sbin/motion.sh to enable it later.\n'
                        ;;
                *)
...
uninstall() {
        ...
        if [ -f "$F_MOTION" ]; then
                if grep -q "$MARK_BEGIN" "$F_MOTION"; then
                        sed "/$MARK_BEGIN/,/$MARK_END/d" "$F_MOTION" > "$TMPD/motion.del"
                        [ -s "$TMPD/motion.del" ] || printf '#!/bin/sh\n' > "$TMPD/motion.del"
                        cat "$TMPD/motion.del" > "$F_MOTION"
                        ok "removed the max hook from $F_MOTION (file kept)"
                else
